...
cd /root/Ascend_operator/custom_ops/PagedAttentionMixV3
bash build.sh

./build_out/custom_opp_ubuntu_aarch64.run --quiet --install-path=/usr/local/Ascend/ascend-toolkit/8.3.RC2/opp

source /usr/local/Ascend/ascend-toolkit/set_env.sh
rm -rf /root/Ascend_operator/custom_ops/PagedAttentionMixV3/build_out
cd /root/Ascend_operator/custom_ops/PagedAttentionMixV3/torch_extension
