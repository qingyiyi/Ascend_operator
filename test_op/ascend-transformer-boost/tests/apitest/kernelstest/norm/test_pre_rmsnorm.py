...
OP_PARAM0 = {"normType": 2, "inBeta": True, "inRes": True, "inGamma": True, "outRes": True, "epsilon": 1e-8,"precisionMode":0}
OP_PARAM1 = {"normType": 2, "inBeta": True, "inRes": True, "inGamma": True, "outRes": True, "epsilon": 1e-8,"precisionMode":1}


class TestPreRmsNorm(op_test.OpTest):
    def golden_calc(self, intensors):
        ...
        if data_type != torch.bfloat16:
            ...
            output1 = inputSum_half
            return [torch.tensor(output0_half).to(data_type), torch.tensor(output1).to(data_type)]
        else:
            if torch.numel(input1) != 0:
            ...
            output1 = inputSum
            return [torch.tensor(output0).to(data_type), torch.tensor(output1).to(data_type)]
    ...
    def golden_compare(self, out_tensors, golden_out_tensors):
        ...
        absolute_error_1 = torch.abs(golden_out_tensors[1].float() - out_tensors[1].float())
        relative_error_1 = absolute_error_1 / golden_out_tensors[1].float()
        cnt_1 = torch.sum(torch.gt(relative_error_1, 0.001)).item()
        logging.debug("cnt", cnt)
        logging.debug("cnt_1", cnt_1)
        return ((cnt / torch.prod(torch.tensor(golden_out_tensors[0].size()))).item() < 0.001) and \
            ((cnt_1 / torch.prod(torch.tensor(golden_out_tensors[1].size()))).item() < 0.001)
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prermsnorm_f16_case000(self):
        '''
            基础场景
        ...
        self.execute([torch.tensor(input0).half(), torch.tensor(input1).half(), torch.tensor(input2).half(),
                      torch.tensor(input3).half()], [torch.zeros(shape0).half(), torch.zeros(shape0).half()])

    @op_test.skip_310b
    @op_test.skip_910a
    def test_prermsnorm_f16_casesmall(self):
        '''
            基础场景(极小数据测试)
            shape: (10, 32) / (1, 32) 
            tilingKey: 000 (short seq, full bias, no bf16)
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prermsnorm_f16_casesmallest(self):
        '''
            基础场景(切块小于32B数据测试)
            shape: (10, 32) / (1, 32) 
            tilingKey: 000 (short seq, full bias, no bf16)
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prermsnorm_f16_case010(self):
        '''
            基础场景
    ...
    @op_test.only_910b
    def test_prermsnorm_bf16_case011(self):
        '''
            基础场景
        ...
        self.execute([torch.tensor(input0).bfloat16(), torch.Tensor().bfloat16(), torch.tensor(input2).bfloat16(),
                      torch.tensor(input3).bfloat16()], [torch.zeros(shape0).bfloat16(), torch.zeros(shape0).bfloat16()])
    
    @op_test.only_910b
    def test_prermsnorm_bf16_case001(self):
        '''
            基础场景
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prermsnorm_f16_larger_case110(self):
        '''
            基础场景
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prermsnorm_f16_larger_case100(self):
        '''
            基础场景
            shape: (100, 15360) / (1, 15360) 
        ...
        self.execute([torch.tensor(input0).half(), torch.tensor(input1).half(), torch.tensor(input2).half(),
                      torch.tensor(input3).half()], [torch.zeros(shape0).half(), torch.zeros(shape0).half()])
    @op_test.only_910b
    def test_prermsnorm_bf16_case101(self):
        '''
            基础场景
        ...
        self.execute([torch.tensor(input0).bfloat16(), torch.tensor(input1).bfloat16(), torch.tensor(input2).bfloat16(),
                      torch.tensor(input3).bfloat16()], [torch.zeros(shape0).bfloat16(), torch.zeros(shape0).bfloat16()])
    @op_test.only_910b
    def test_prermsnorm_bf16_case111(self):
        '''
            基础场景
            shape: (100, 15360) / (1, 15360) 
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prermsnorm_f16_case1(self):
        ...
        self.execute([torch.tensor(input0).half(), torch.tensor(input1).half(), torch.tensor(input2).half(),
                      torch.tensor(input3).half()], [torch.zeros(shape0).half(), torch.zeros(shape0).half()])
    @op_test.only_910b
    def test_prenorm_b16_caserand_with_bias(self):
        for i in range(5):
            logging.info("bias and bf16 idx shape is %d", i)
            ...
            self.execute([torch.tensor(input0).bfloat16(), torch.tensor(input1).bfloat16(), torch.tensor(input2).bfloat16(),
                          torch.tensor(input3).bfloat16()], [torch.zeros(shape0).bfloat16(), torch.zeros(shape0).bfloat16()])
    @op_test.only_910b
    def test_prenorm_b16_caserand_no_bias(self):
        for i in range(5):
            logging.info(" no bias and bf16 idx shape is %d", i)
            Shape0 = torch.randint(1,512,(1,))
            randVal = torch.randint(16,20480,(1,))
            shape0 = (Shape0[0], randVal[0])
            shape1 = (1, randVal[0])
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prenorm_fp16__no_bias(self):
        for i in range(5):
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prenorm_fp16_caserand_bias(self):
        for i in range(5):
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prenorm_fp16_caserand_bias_high_perf(self):
        for i in range(5):
            logging.debug("bias and fp16 idx shape is %d", i)
            Shape0 = torch.randint(1,512,(1,))
            randVal = torch.randint(16,20480,(1,))
    ...
    @op_test.skip_310b
    @op_test.skip_910a
    def test_prenorm_fp16__no_bias_high_perf(self):
        for i in range(5):
            logging.debug(" no bias and fp16 idx shape is %d", i)
            Shape0 = torch.randint(1,512,(1,))
            randVal = torch.randint(16,20480,(1,))
    ...
    @op_test.only_910b
    def test_prermsnorm_f16_tencent_wxg_case(self):
        shape0 = (3000, 2560)
        shape1 = (1, 2560)
        input0 = torch.empty(shape0, dtype=torch.half).uniform_(0, 256)
        input1 = torch.empty(shape1, dtype=torch.half).uniform_(0, 256)
