...
class TestPagedAttentionMLA(op_test.OpTest):
    ...
    def calc_data(self, num_tokens, q_seqlen, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks, k_seqlen,\
        ...
        if self.is_quant_flag:
            ...
            self.key_cache_split2 = key_cache_split2_nz.reshape(num_blocks, -1, block_size, 16)         
        elif not is_nz_in:
    ...
    @op_test.only_910b
    def test_paged_mla_split_cache_norm_flash_decoding_test1(self):
        self.set_support_910b_only()
        num_tokens = 32
        q_seqlen = 2
        k_seqlen = 4096
        q_seqlen_list = [q_seqlen] * num_tokens
        k_seqlen_list = [4096, 4097, 3841, 3500] * 8
        num_heads = 128
        kv_heads = 1
        block_size = 128
        head_size_qk = 576
        ...
        self.calc_data(num_tokens, q_seqlen, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks, k_seqlen, dtype, mask_dim, torch.float16,
                        is_kv_combined = is_kv_combined, is_quant_flag = is_quant_flag, fa_block_size = fa_block_size, dynamic_batch = True, dynamic_seqlen = k_seqlen_list)
        OP_NAME = "MLAOperation"
        OP_PARAM = {"type": 0, "kvHead": kv_heads, "headSize": num_heads, "tor": tor,
                    "kvSeqLen": k_seqlen_list, "qSeqLen": q_seqlen_list, "maskType": 0, "isRing": 0}
    ...
    @op_test.only_910b
    def test_paged_mla_split_cache_norm_flash_decoding_test2(self):
        self.set_support_910b_only()
        num_tokens = 8
        q_seqlen = 1
        k_seqlen = 4096
        q_seqlen_list = [q_seqlen] * num_tokens
        k_seqlen_list = [4097, 4096, 3978, 3700] * 2
        num_heads = 128
        kv_heads = 1
        block_size = 128
        head_size_qk = 576
    ...
    @op_test.only_910b
    def test_paged_mla_split_cache_norm_flash_decoding_test3(self):
        self.set_support_910b_only()
        num_tokens = 32
        q_seqlen = 2
        k_seqlen = 4096
    ...
    def test_paged_mla_split_cache_quant_fp16_q4_numheads32_quant_ring(self):
        ...
        lse = torch.zeros(shape_out_2, dtype = torch.float32)
        for i in range (1):
            self.execute(
            [
                self.q_split1,
    ...
    @op_test.only_910b
    def test_paged_mla_split_cache_quant_fp16_q1_numheads8_quant_ring(self):
        self.set_support_910b_only()
        num_tokens = 32
        q_seqlen = 1
        k_seqlen = 1700
    ...
    @op_test.only_910b
    def test_paged_mla_split_cache_quant_bf16_q1_numheads128_quant_ring(self):
        self.set_support_910b_only()
        num_tokens = 8
        q_seqlen = 1
        k_seqlen = 1700
