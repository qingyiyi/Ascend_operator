...
class TestPagedAttentionMLA(op_test.OpTest):
    ...
    @op_test.only_910b
    def test_paged_mla_split_cache_large_groupnum(self):
        num_tokens = 20
        num_heads = 128
        kv_heads = 1
        block_size = 128
    ...
    @op_test.only_910b
    def test_paged_mla_split_cache_norm(self):
        num_tokens = 20
        num_heads = 4
        kv_heads = 1
        block_size = 128
    ...
    @op_test.only_910b
    def test_paged_mla_combine_cache_norm1(self):
    ...
    @op_test.only_910b4
    def test_paged_mla_quant_case(self):
        self.set_support_910b_only()
        num_tokens = 20
        num_heads = 40
        kv_heads = 1
    ...
    @op_test.only_910b4
    def test_paged_mla_quant_case2(self):
        self.set_support_910b_only()
        num_tokens = 16
        num_heads = 128
        kv_heads = 1
    ...
    @op_test.only_910b4
    def test_paged_mla_quant_unaligned_embed(self):
        self.set_support_910b_only()
        for i in range (1):
            num_tokens = 70
            num_heads = 25
            ...
            head_size_vo = 130
            head_size_qk = 290
            logging.info(f"head_size_qk : {head_size_qk}")
            logging.info(f"head_size_vo : {head_size_vo}")
            num_blocks = 64 
            dynamic_batch = False
            k_seqlen = 128
            tor = 1.0 / (head_size_qk ** 0.5)
            ...
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead":kv_heads, "headSize":num_heads, "tor": tor, "headDimV" :head_size_vo, "maskType": 0, "kvSeqLen": self.contex_lens.tolist(), "quantType": 3, 'outDataType':1}
            self.set_param(OP_NAME, OP_PARAM)
            self.set_input_formats([self.format_nd] * 9)
            self.set_output_formats([self.format_nd])
            shape_out = ((num_tokens, num_heads, head_size_vo))
    ...
    @op_test.only_910b4
    def test_pa_mla_quantoffline_case_normal_mask(self):
        self.set_support_910b_only()
        num_tokens = 9
        num_heads = 128
        kv_heads = 1
    ...
    @op_test.only_910b4
    def test_pa_quant_mla_case_normal_mask(self):
        num_tokens = 9
        num_heads = 128
        kv_heads = 1
        block_size = 128
        ...
        is_quant_flag = 1
        is_kv_combined = True
        self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks, k_seqlen, dtype, mask_dim, torch.float16, dynamic_batch = True, dynamic_seqlen = k_seqlen,is_kv_combined = is_kv_combined, is_quant_flag = is_quant_flag)
        OP_NAME = "PagedAttentionOperation"
        OP_PARAM = {"type": 2005, "kvHead":kv_heads, "headSize":num_heads, "tor": tor, "headDimV" :head_size_vo, "maskType": 1, "kvSeqLen": self.contex_lens.tolist(), "quantType": 3, 'outDataType':1}
        self.set_param(OP_NAME, OP_PARAM)
        self.set_input_formats([self.format_nd] * 9)
        self.set_output_formats([self.format_nd])
        shape_out = ((num_tokens, num_heads, head_size_vo))
    ...
    @op_test.only_910b
    def test_paged_mla_combine_cache_norm_256(self):
        self.set_support_910b_only()
        num_tokens = 1
        num_heads = 16
        kv_heads = 1
