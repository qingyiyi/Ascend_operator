...
class TestPagedAttentionMLA(operation_test.OperationTest):
    ...
    def softmax_numpy(self, sim):
        ...
        soft_res = sim_sub / row_sum
        if (type(self.rowmaxs) is np.ndarray and type(self.rowsums) is np.ndarray):
            self.rowmaxs = np.concatenate((self.rowmaxs, np.expand_dims(row_max, axis=0)), axis=0)
            self.rowsums = np.concatenate((self.rowsums, np.expand_dims(row_sum, axis=0)), axis=0)
        else:
            self.rowmaxs = np.expand_dims(row_max, axis=0)
            self.rowsums = np.expand_dims(row_sum, axis=0)
        return soft_res
    ...
    def ref_single_query_cached_kv_attention(self,
        ...
        for i in range(len(context_lens)):
            ...
            if mask_dim == 4:
                out, out_high, sim_out = self.ref_masked_attention(q, keys, values, scale, mask[i, :, :, :context_len],
                                                                   mask_data_type)
                out = out.reshape(num_heads, head_size_vo)
            elif mask_dim == 3:
                out, out_high, sim_out = self.ref_masked_attention(q, keys, values, scale,
                                                                   mask[i // mask_index_coff, :, :context_len],
                                                                   mask_data_type)
                out = out.reshape(num_heads, head_size_vo)
            else:
                out, out_high, sim_out = self.ref_masked_attention(q, keys, values, scale, mask, mask_data_type)
                out = out.reshape(num_heads, head_size_vo)
    ...
    def calc_data(self, num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks, k_seqlen, \
                  ...
                  compressHead=False, is_kv_combined=True, calcType=0):
        self.rowmaxs = -1
        self.rowsums = -1
    ...
    def golden_calc(self, in_tensors):
        ...
        result = [golden_out]
        if self.calcType:
            lse_golden = torch.tensor(np.log(self.rowsums) + self.rowmaxs).squeeze(-1).to(self.data_type)
            result.append(lse_golden)
        return result
    ...
    def golden_compare(self, out_tensors, golden_tensors):
        if self.compare_count == 0:
            ...
            result_double = compare_cv(golden_tensors.npu(), golden_tensors.npu(), out_tensors.npu())
            result_old = self.compare_output_data(out_tensors.npu(), golden_tensors.npu(), [0.001, 0.001, 0.005, 0.005])
        self.compare_count += 1
        return (result_double or result_old)

    def test_paged_mla_combine_cache_calcType(self):
        ...
        self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks, k_seqlen,
                       dtype, mask_dim, torch.float16,
                       is_kv_combined=is_kv_combined, calcType=calcType)

        OP_NAME = "MultiLatentAttentionOperation"
        PARAM = json.dumps({"headNum": num_heads, "qkScale": tor, "kvHeadNum": kv_heads, "maskType": 0, "calcType": calcType, "cacheMode": 1})
    ...
    def test_paged_mla_combine_cache_bf16(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        self.compare_count = 0
