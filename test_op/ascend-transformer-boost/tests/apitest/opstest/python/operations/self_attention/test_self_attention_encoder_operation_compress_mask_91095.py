...
def calc_expect_func_950(batch, seqlen, heads, embed, headDims_v, group_num=32, pseShiftFlag = False, is_mask = False):
    ...
    mask = np.triu(mask, 1)
    mask_kernel = np.ones(shape=(2048, 2048)).astype(np.int8)  # 使用当前最大seqlen生成mask
    mask_kernel = np.triu(mask_kernel, 1)
    print(mask)
    ...
    mask_kernel = mask_kernel.astype(mask_type).reshape(2048, 2048)
    q_len = q_seqlen.astype(np.int32)
    out = out.astype(src_type).reshape(-1, heads, headDims_v)

    ret_data_list = [q, k, v]
    if is_mask:
        ret_data_list.append(mask_kernel)
...
if operation_test.get_soc_version() == 'Ascend950':
    ...
    PARAM = json.dumps({"headNum": 2, "qkScale": (1 / float(math.sqrt(16))), "kvHeadNum": 2, \
        "maskType": 1, "calcType": 3, "isTriuMask": 1})
