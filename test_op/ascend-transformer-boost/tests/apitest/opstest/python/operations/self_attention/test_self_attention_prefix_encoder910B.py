...
class TestFlashAttentionPrefixEncoder(operation_test.OperationTest):
    ...
    def compare_output_data(self, out, golden, ratios):
        ...
        logging.info("1/1000 Accuracy is %f",  1 -
                     float(error_count) / out_len)
        logging.info("5/1000 Accuracy is %f",  1 -
                     float(strict_error_count) / out_len)
        if self.data_type == torch.bfloat16:
            logging.debug("accuracy is correct in old standard: %r",
    ...
    def test_flash_attention_case_fa_encoder_withcache_bf16_norm_compress_128(self):
        ...
        OP_PARAM = {"type": UNPAD_FLASH_ATTENTION_ENCODER_PREFIX_CACHE_ND, "qSeqLen": q_seqlens, "kvSeqLen": kv_seqLen, "headSize": heads, "tor": tor,
                    "isClamp": is_clamp, "clampMin": clamp_min, "clampMax": clamp_max, "maskType": MASK_TYPE_NO_BATCH, "kvHead": kv_head,
        ...
        PARAM = json.dumps(
            {"headNum": heads, "calcType": CAL_TYPE_PREFIX_ENCODER, "maskType": 3,
        ...
        ), self.block_tables.npu(), self.mask.to(data_type).npu(), q_seqlen, kv_seqLen])
        
    def test_flash_attention_case_fa_encoder_withcache_fp16_norm_compress_128(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # [b,ms,ms]
    ...
    def test_flash_attention_case_fa_encoder_withcache_bf16_norm_compress_128_single_batch(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # [b,ms,ms]
    ...
    def test_flash_attention_case_fa_encoder_withcache_fp16_norm_compress_128_single_batch(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # [b,ms,ms]
    ...
    def test_flash_attention_case_fa_encoder_withcache_bf16_alibi_sqrt_single_batch_without_tail_block(self):
        ...
        num_blocks = 1024
        OP_NAME = "SelfAttentionOperation"
        OP_PARAM = {"type": UNPAD_FLASH_ATTENTION_ENCODER_PREFIX_CACHE_ND, "qSeqLen": q_seqlens, "kvSeqLen": kv_seqLen,
                    "headSize": heads, "tor": tor, "maskType": MASK_TYPE_ALIBI_COMPRESS_SQRT, "kvHead": kv_head,
        ...
        PARAM = json.dumps(
            {"headNum": heads, "calcType": CAL_TYPE_PREFIX_ENCODER, "maskType": MASK_TYPE_ALIBI_COMPRESS_SQRT,
             "kvHeadNum": kv_head, "isTriuMask": 1, "qkScale": tor, "kernelType": KERNELTYPE_HIGH_PRECISION})
        RUN_PARAM = json.dumps({"seqLen": q_seqlens, "kvSeqLen": kv_seqLen, "CalcType": CAL_TYPE_PREFIX_ENCODER})
        q_seqlen = np.array(q_seqlens)
        q_seqlen = torch.from_numpy(q_seqlen).to(torch.int32).npu()
        ...
        self.v_cache = self.v_cache.view(num_blocks, block_size, kv_head * embeddim)
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.npu(), self.k_cache.npu(), self.v_cache.npu(
        ), self.block_tables.npu(), self.mask.to(data_type).npu(), q_seqlen, kv_seqLen, self.alibi_slopes.to(torch.float32).npu()])

    def test_flash_attention_case_fa_encoder_withcache_bf16_alibi_compress_single_batch_without_tail_block(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # [b,ms,ms]
        ...
        OP_NAME = "SelfAttentionOperation"
        OP_PARAM = {"type": UNPAD_FLASH_ATTENTION_ENCODER_PREFIX_CACHE_ND, "qSeqLen": q_seqlens, "kvSeqLen": kv_seqLen,
                    "headSize": heads, "tor": tor, "maskType": MASK_TYPE_ALIBI_COMPRESS, "kvHead": kv_head,
                    "isTriuMask": 1, "alibiLeftAlign": 0, "isAlibiMaskSqrt": 0}
        data_type = torch.bfloat16
        ...
        self.mask = self.mask.to(torch.bfloat16)
        PARAM = json.dumps(
            {"headNum": heads, "calcType": CAL_TYPE_PREFIX_ENCODER, "maskType": MASK_TYPE_ALIBI_COMPRESS,
             "kvHeadNum": kv_head, "isTriuMask": 1, "qkScale": tor, "kernelType": KERNELTYPE_HIGH_PRECISION})
        RUN_PARAM = json.dumps({"seqLen": q_seqlens, "kvSeqLen": kv_seqLen, "CalcType": CAL_TYPE_PREFIX_ENCODER})
        q_seqlen = np.array(q_seqlens)
        q_seqlen = torch.from_numpy(q_seqlen).to(torch.int32).npu()
    ...
    def test_flash_attention_case_fa_encoder_withcache_bf16_alibi_mask128_single_batch_without_tail_block(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # [b,ms,ms]
        ...
        OP_NAME = "SelfAttentionOperation"
        OP_PARAM = {"type": UNPAD_FLASH_ATTENTION_ENCODER_PREFIX_CACHE_ND, "qSeqLen": q_seqlens, "kvSeqLen": kv_seqLen, "headSize": heads, "tor": tor,
                    "isClamp": is_clamp, "clampMin": clamp_min, "clampMax": clamp_max, "maskType": MASK_TYPE_ALIBI_COMPRESS, "kvHead": kv_head,
                    "isTriuMask": 1, "alibiLeftAlign": 0, "isAlibiMaskSqrt": 0}
        data_type = torch.bfloat16
    ...
    def test_flash_attention_case_fa_encoder_withcache_bf16_alibi_sqrt_single_batch_with_tail_block(self):
       ...
       # [b,ms,ms]·
        ...
        OP_NAME = "SelfAttentionOperation"
        OP_PARAM = {"type": UNPAD_FLASH_ATTENTION_ENCODER_PREFIX_CACHE_ND, "qSeqLen": q_seqlens, "kvSeqLen": kv_seqLen, "headSize": heads, "tor": tor,
                    "maskType": MASK_TYPE_ALIBI_COMPRESS_SQRT, "kvHead": kv_head, "isTriuMask": 1, "alibiLeftAlign": 0, "isAlibiMaskSqrt": 1}
        data_type = torch.bfloat16
