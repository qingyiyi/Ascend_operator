...
class TestFlashAttentionQuant(operation_test.OperationTest):
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_online_split_seqlen(self):
        ...
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 3, "outDataType": 27})
        ...
        RUN_PARAM = json.dumps({"seqLen": param_seqlen, "quantType": 3})
        ...
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q_int8.npu(), self.k_int8.npu(), self.mask.to(data_type).npu(), seqlen.npu(),
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_online(self):
        ...
        RUN_PARAM = json.dumps({"seqLen": kv_seqLen, "quantType": 3})
        kv_seqLen = np.array(kv_seqLen)
        kv_seqLen = torch.from_numpy(kv_seqLen).to(torch.int32).npu()
        attention_out = np.zeros_like(self.golden_out.to(torch.float16))
        
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q_int8.npu(), self.k_int8.npu(), self.mask.to(data_type).npu(), kv_seqLen,
                            torch.tensor(self.q_scale * self.k_scale, dtype=torch.float).npu(),
                            torch.tensor([1], dtype=torch.int).npu(),
                            torch.tensor(self.v_scale, dtype=torch.float).npu(),
                            torch.tensor([1], dtype=torch.int).npu()])
        
    def test_flash_attention_case_fa_encoder_mla_high_precision_bf16_fav3_int8_online(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.gen_out_tensor(online)
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 3, "outDataType": 27})
    ...
    def test_flash_attention_case_fa_encoder_mla_bf16_no_mask_int8_online(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.gen_out_tensor(online)
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 3, "outDataType": 27})
        ...
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q_int8.npu(), self.k_int8.npu(), kv_seqLen,
                            torch.tensor(self.q_scale * self.k_scale, dtype=torch.float).npu(),
                            torch.tensor([1], dtype=torch.int).npu(),
                            torch.tensor(self.v_scale, dtype=torch.float).npu(),
                            torch.tensor([1], dtype=torch.int).npu()])
        
    def test_flash_attention_case_fa_encoder_mla_high_precision_bf16_no_mask_int8_online(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.gen_out_tensor(online)
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 3, "outDataType": 27})
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_offline(self):
        ...
        self.gen_out_tensor()
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 2, "outDataType": 27})
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_offline_high_precision(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.gen_out_tensor()
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 2, "outDataType": 27})
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_offline_mask0(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.gen_out_tensor()
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 2, "outDataType": 27})
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_offline_high_precision_mask0(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.gen_out_tensor()
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 2, "outDataType": 27})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_online(self):
        ...
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 3, "outDataType": 1})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_online_high_precision(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.set_data_params(dynamic_batch = dynamic_batch, 
                             ...
                             op_type = OP_PARAM["type"], mask_type = MASK_TYPE_NO_BATCH, no_cache = True, fav3=fav3, tor = tor)
        self.gen_out_tensor(online)
        mask = np.ones(shape=(2048,2048)) * 1
        mask = np.triu(mask, 1)
        self.mask = torch.from_numpy(mask)
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 3, "outDataType": 1})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_online_mask0(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.mask = np.reshape(self.mask, (max_seq, max_seq))
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 3, "outDataType": 1})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_online_mask0_high_precision(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.mask = np.reshape(self.mask, (max_seq, max_seq))
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 3, "outDataType": 1})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_offline(self):
        ...
        self.mask = np.reshape(self.mask, (max_seq, max_seq))
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 2, "outDataType": 1})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_offline_high_precision(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.gen_out_tensor()
        # self.mask = np.reshape(self.mask, (max_seq, max_seq))
        mask = np.ones(shape=(2048,2048)) * 1.0
        mask = np.triu(mask, 1)
        self.mask = torch.from_numpy(mask)
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 2, "outDataType": 1})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_offline_mask0(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.mask = np.reshape(self.mask, (max_seq, max_seq))
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 2, "outDataType": 1})
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_offline_mask0_high_precision(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        # unpad encoder
        ...
        self.mask = np.reshape(self.mask, (max_seq, max_seq))
        PARAM = json.dumps({"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
            "calcType": 3, "maskType": 0, "mlaVHeadSize": embeddimV, "kernelType": 1, "quantType": 2, "outDataType": 1})
