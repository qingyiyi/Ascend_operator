...
if scaleType:
    in_tensors[6] = torch.tensor(in_tensors[6], dtype=torch.float16)
...
PARAM = json.dumps({"headNum": 32, "qkScale": (1 / float(math.sqrt(128))), "kvHeadNum": kv_head,
                    "calcType": 3, "scaleType": scaleType})
RUN_PARAM = json.dumps({"seqLen": param_seqlen,"scaleType": scaleType})# "scaleType" is used for correct binder count.If anyelse is added, please put it at the end of run_param.
