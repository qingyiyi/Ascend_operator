...
def lccl_sum_cal(inTensors):
    ...
    for i in range(len(inTensors)):
        cal_tensors[i] = inTensors[i]
    result = ((cal_tensors[0] + cal_tensors[1])+(cal_tensors[2]+cal_tensors[3]))+((cal_tensors[4] + cal_tensors[5])+(cal_tensors[6]+cal_tensors[7]))
    return result
...
class AllReduceOperationTest(operation_test.OperationTest):
    def test_all_reduce(self):
        ...
        if res == 0:
            world_size = 2
            random_seed = 123
            inTensorDtypes = [torch.int8, torch.int16, torch.int32,torch.float32,torch.float16, torch.bfloat16]
            sizes = [[10,100,512]]
