...
def get_soc_version():
    ...
    if (re.search("Ascend910B", device_name, re.I) and len(device_name) > 10) or re.search("Ascend910_93",
                                                                                            device_name, re.I):
    ...
    print("Device Properties: device_name: {}, soc_version: {}, device_count: {}, current_device: {}".format(device_name, soc_version, device_count, current_device))
    return soc_version
...
class OperationTest(unittest.TestCase):
    ...
    def execute_split_thread(self, op_name, op_param, run_param, in_tensors):
        print(f"———————— {op_name} test start ————————")
        self.operation = torch.classes.OperationTorch.OperationTorch(
            op_name)
        if isinstance(op_param, dict):
            ...
            self.operation.set_param(op_param)
        self.operation.set_varaintpack_param(run_param)
        out_tensors = self.operation.execute_split_thread(in_tensors)
        print("out_tensor", out_tensors[0].size())
        golden_out_tensors = self.golden_calc(in_tensors)
        print("golden_calc", golden_out_tensors[0].size())
        self.__golden_compare_all(out_tensors, golden_out_tensors)
