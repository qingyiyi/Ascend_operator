...
class TestPagedAttentionDataGenerator():
    ...
    def get_blockszie_calc(self, max_context_len, block_size, embeddingSize, embeddingSizeV):
        ...
        if self.is_quant_flag and \
            ...
            KV_SEQLEN_SLICE_512 * np.maximum(embedQKSplit, embedVOSplit) <= BLOCK_LIMIT_NO_PINGPONG and self.head_num_move < 4:
...
PARAM = json.dumps({"headNum":32, "qkScale": (1 / 128 ** 0.5), "kvHeadNum": 2, "maskType": 1, "compressType": 0, "quantType": 3, "outDataType": 27})
...
class TestPagedAttentionAttentionOperationCompressHead(operation_test.OperationTest):
    ...
    def test(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            ...
            return
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [in_tensors[0], in_tensors[1], in_tensors[2], in_tensors[3], in_tensors[4], in_tensors[5], in_tensors[6], in_tensors[7]])
