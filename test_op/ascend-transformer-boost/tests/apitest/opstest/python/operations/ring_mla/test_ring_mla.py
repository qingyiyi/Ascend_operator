...
def ref_flash_attention(
    ...
    return torch.from_numpy(go), torch.from_numpy(go_high), gl, gm
...
class TestMLAPrefill(operation_test.OperationTest):
    ...
    def set_data_params(self, dynamic_batch=False, batch_state=None, window_size=0, cache_type=0,
        ...
        if self.fav3:
            ...
            self.q_int8 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(self.q_ntokens, heads * self.embeddim))).to(torch.int8)
            self.k_int8 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(self.layer_id[0] + 1, batch, self.max_seq, kv_head * self.embeddim))).to(torch.int8)
            self.v_int8 = torch.from_numpy(np.random.uniform(-1.0, 1.0, size=(self.layer_id[0] + 1, batch, self.max_seq, kv_head * self.embeddimv))).to(torch.int8)
    ...
    def gen_out_tensor(self, online=False):
        ...
        for idx in range(batch):
            ...
            new_out, new_out_height, gl, gm = ref_flash_attention(q_slice_ori, k_slice_ori, v_slice_ori, self.tor, temp_mask, context_len=context_len, mask_type=self.mask_type)
        ...
        # golden data
        self.new_les = torch.from_numpy(np.array(self.new_les)) #  (bs, head, 1)
        self.new_les = torch.squeeze(self.new_les, dim=-1)
        self.new_les = self.new_les.permute(1, 0).contiguous()
        ...
        if self.is_int8_flag:
            ...
            if not self.isring:
                ...
                out = out.view(q_ntokens, heads * embedv)
                new_out, new_lse = self.update_out(out, self.new_les)
                self.golden_out = new_out.to(self.data_type)
                self.new_les_low = new_lse

                out_true = out_height.view(q_ntokens, heads * embedv)
                new_out, new_lse_height = self.update_out(out_true, self.new_les)
    ...
    def golden_calc(self, in_tensors):
        golden_out = self.golden_out.clone().detach().npu()
        if self.isring == 0:
            lse_result = self.new_les.clone().detach().npu()
        else:
            lse_result = self.new_lse_height.clone().detach().npu()
        self.index = 0
        return [golden_out, lse_result]

    def golden_compare(self, out_tensors, golden_tensors):
        result_single = self.compare_output_data(out_tensors.half(), golden_tensors.half(), [0.001, 0.001, 0.005, 0.005])
        result_single_lse = self.compare_output_data(out_tensors.half(), golden_tensors.half(), [0.001, 0.001, 0.005, 0.005])
        if self.isring == 0:
            lse_result = self.new_les.clone().detach().npu()
        else:
            lse_result = self.new_lse_height.clone().detach().npu()
        ...
        if self.index % 2 == 0:
            result_double = compare_cv(lse_result.flatten().cpu(), golden_tensors.flatten().cpu(), out_tensors.flatten().cpu())
            return result_double or result_single_lse
        else:
            result_double = compare_cv(self.golden_out_true.flatten().cpu(), golden_tensors.flatten().cpu(), out_tensors.flatten().cpu())
            return result_double or result_single

    def test_flash_attention_mla_fp16_no_mask(self):
        '''
        embeddim > embeddimV, no mask, q_seqlen == kv_seqLen, only input kv_seqLen, isring = 0
        '''
        ...
        # isring = 1
        shape_out_1 = (sum(kv_seqLen), heads, embeddimV)  # sum(q_seq), head*ebeddimv
        ...
        self.gen_out_tensor()
        logging.info("case: test_flash_attention_mla_fp16_no_mask")
        ...
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q_split1.reshape(q_ntokens, heads, 128).npu(),
                                                            ...
                                                            self.mask.to(data_type).reshape(max_seq, max_seq).npu(), seqlen.npu(),
                                                        ])


    def test_flash_attention_mla_fp16_512_no_mask_is_ring(self):
        '''
        embeddim > embeddimV, no mask, q_seqlen == kv_seqLen, only input kv_seqLen, isring = 1
        '''
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        ...
        OP_NAME = "RingMLAOperation"
        OP_PARAM = {"type": 1, "qSeqLen":kv_seqLen, "kvSeqLen": kv_seqLen, "headDimV": embeddimV,"headSize": heads,
                    "tor": tor, "isTriuMask": 0, "maskType": 0, "kvHead": heads, "isRing":isring}
        self.set_data_params(dynamic_batch = dynamic_batch,
                            is_decoder = isdecoder, batch = batch, kv_head = kv_head, heads = heads,
                            embeddim = embeddim,embeddimv = embeddimV, max_seq = max_seq, kv_seqLen = kv_seqLen,
                            is_clamp = is_clamp, clamp_max = clamp_max, clamp_min = clamp_min,
        ...
        logging.info("case: test_flash_attention_mla_fp16_512_no_mask_is_ring")
        logging.info("**********input shape***********")
        logging.info(f"q shape: {self.q.shape}")
        logging.info(f"k shape: {self.k.shape}")
        logging.info(f"v shape: {self.v.shape}")
    ...
    def test_flash_attention_mla_fp16_512_no_mask_multibatch_is_ring(self):
        '''
        embeddim > embeddimV, no mask, q_seqlen == kv_seqLen, only input kv_seqLen, isring = 1
        '''
        if not operation_test.get_soc_version() == 'Ascend910B':
        ...
        logging.info("case: test_flash_attention_mla_fp16_512_no_mask_multibatch_is_ring")
        logging.info("**********input shape***********")
        logging.info(f"q shape: {self.q.shape}")
        logging.info(f"k shape: {self.k.shape}")
        logging.info(f"v shape: {self.v.shape}")
    ...
    def test_flash_attention_mla_bf16_512_mask_is_ring(self):
        '''
        embeddim > embeddimV, mask, q_seqlen > kv_seqLen, isring = 1
        '''
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        ...
        shape_out_1 = (sum(q_seqLen), heads, embeddimV)  # sum(q_seq), head*ebeddimv
        ...
        OP_NAME = "RingMLAOperation"
        OP_PARAM = {"type": 1, "qSeqLen":q_seqLen, "kvSeqLen": kv_seqLen, "headDimV": embeddimV,"headSize": heads,
                    "tor": tor, "isTriuMask": 0, "maskType": 0, "kvHead": heads, "isRing":isring}
        self.set_data_params(dynamic_batch = dynamic_batch,
                            is_decoder = isdecoder, batch = batch, kv_head = kv_head, heads = heads,
                            embeddim = embeddim,embeddimv = embeddimV, max_seq = max_seq, kv_seqLen = kv_seqLen,
        ...
        logging.info("case: test_flash_attention_mla_bf16_512_mask_is_ring")
        logging.info("**********input shape***********")
        logging.info(f"q shape: {self.q.shape}")
        logging.info(f"k shape: {self.k.shape}")
        logging.info(f"v shape: {self.v.shape}")
        ...
        PARAM = json.dumps(
                {"headNum": heads, "calcType": calcType, "maskType": MASK_TYPE_NO_HEAD,
                "kvHeadNum": kv_head, "qkScale": tor})
    ...
    def test_flash_attention_mla_bf16_512_mask_is_ring_kvSeqlen_0(self):
        '''
        embeddim > embeddimV, mask, q_seqlen != kv_seqLen, kv_seqLen[i] = 0, isring = 1
        '''
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        ...
        logging.info("case: test_flash_attention_mla_bf16_512_mask_is_ring_kvSeqlen_0")
        logging.info("**********input shape***********")
        logging.info(f"q shape: {self.q.shape}")
        logging.info(f"k shape: {self.k.shape}")
        logging.info(f"v shape: {self.v.shape}")
    ...
    def test_flash_attention_mla_bf16_512_mask_is_ring_diff_kvSeqlen(self):
        '''
        embeddim > embeddimV, mask, q_seqlen != kv_seqLen, kv_seqLen[i] = kv_seqLen[j], isring = 1
        '''
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        ...
        OP_NAME = "RingMLAOperation"
        OP_PARAM = {"type": 1, "qSeqLen":q_seqLen, "kvSeqLen": kv_seqLen, "headDimV": embeddimV,"headSize": heads,
                    "tor": tor, "isTriuMask": 1, "maskType": 1, "kvHead": heads, "isRing":isring}
        self.set_data_params(dynamic_batch = dynamic_batch,
                            is_decoder = isdecoder, batch = batch, kv_head = kv_head, heads = heads,
                            embeddim = embeddim,embeddimv = embeddimV, max_seq = max_seq, kv_seqLen = kv_seqLen,
                            is_clamp = is_clamp, clamp_max = clamp_max, clamp_min = clamp_min,
        ...
        logging.info("case: test_flash_attention_mla_bf16_512_mask_is_ring_diff_kvSeqlen")
        logging.info("**********input shape***********")
        logging.info(f"q shape: {self.q.shape}")
        logging.info(f"k shape: {self.k.shape}")
        logging.info(f"v shape: {self.v.shape}")
        ...
        attention_out = np.ones_like(self.golden_out.to(torch.float16))
    ...
    def test_flash_attention_mla_bf16_512_mask_is_ring_kvSeqlen_0(self):
        ...
        calcType = 0
        if isring:
            old_out = torch.zeros(shape_out_1).to(data_type)
            old_lse = (torch.zeros(shape_out_2) * 10).to(torch.float32)
        else:
            old_out = torch.zeros(shape_out_1, dtype=data_type)
            old_lse = torch.zeros(shape_out_2, dtype=torch.float32)
            calcType = 1
    ...
    def test_flash_attention_mla_bf16_512_mask_is_ring_maxq_gt_maxkv(self):
        '''
        embeddim > embeddimV, mask, max(q_seqlen) > max(kv_seqlen), kv_seqLen[i] = 0, isring = 1
        '''
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
