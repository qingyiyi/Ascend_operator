...
class TestPagedAttentionDataGenerator():
    ...
    def ref_masked_attention(self,
        ...
        if self.is_quant_flag:
            p_high, row_sum, de_scalev, _, _ = self.softmax_quant_numpy(sim_high.numpy(),
                                                                        np.full([query.shape[0], 1, 1],
                                                                                np.finfo(np.float32).min), 1)
            self.de_scalev = de_scalev
    ...
    def get_blockszie_calc(self, max_context_len, block_size, embeddingSize, embeddingSizeV):
        ...
        if self.is_quant_flag and \
                ...
                KV_SEQLEN_SLICE_512 * np.maximum(embedQKSplit,
                                                 embedVOSplit) <= BLOCK_LIMIT_NO_PINGPONG and self.head_num_move < 4:
    ...
    def calc_data(self, num_tokens, num_heads, kv_heads, head_size, block_size, num_blocks, k_seqlen, \
                  ...
                  compressHead=False, razor_rope=False, blocknum=20, is_quant_flag=0):
        ...
        if is_int8_flag:
            kv_range = 4.0
            kv_type = torch.int8
        query = torch.from_numpy(np.random.uniform(-q_range, q_range, size=(num_tokens, num_heads, head_size))).to(
            dtype)
        # (num_blocks, block_size, num_heads, head_size)
        if not compressHead:
            key_cache = torch.from_numpy(
                np.random.uniform(-kv_range, kv_range, size=(num_blocks, block_size, kv_heads, head_size))).to(kv_type)
            # # (num_blocks, block_size, num_heads, head_size)
            ...
            key_cache = torch.from_numpy(
                np.random.uniform(-kv_range, kv_range, size=(num_blocks * kv_heads, block_size, 1, head_size))).to(
                kv_type)
            # # (num_blocks, block_size, num_heads, head_size)
            value_cache = torch.from_numpy(
                np.random.uniform(-kv_range, kv_range, size=(num_blocks * kv_heads, block_size, 1, head_size))).to(
        ...
        if self.is_quant_flag:
            self.de_scale1_fp32 = torch.from_numpy(np.random.uniform(-1, 2, size=(num_heads)).astype(np.float32)).to(
                torch.float32)
            self.de_scale2_fp32 = torch.from_numpy(np.random.uniform(-1, 2, size=(num_heads)).astype(np.float32)).to(
                torch.float32)
            self.scale = torch.from_numpy(np.random.uniform(-1, 2, size=(num_heads)).astype(np.float32)).to(
                torch.float32)
    ...
    def test_pa_quant_case_normal_mask(self):
        ...
        self.calc_data(num_tokens, num_heads, kv_heads, head_size, block_size, num_blocks, k_seqlen, dtype, mask_dim,
                       outDtype, dynamic_batch, k_seqlen, is_quant_flag=is_quant_flag)
    ...
    def test_pa_quantoffline_case_normal_mask(self):
        ...
        is_quant_flag = 1
        self.calc_data(num_tokens, num_heads, kv_heads, head_size, block_size, num_blocks, k_seqlen, dtype, mask_dim,
                       outDtype, dynamic_batch, k_seqlen, is_quant_flag=is_quant_flag, is_quant_offiline=1)
        attention_out = torch.zeros_like(self.q).to(outDtype)
        attention_out[:] = 0.1
...
(q, k, v, block_table, context_lens, mask, kDescale, vDescale) = (
    data_generator.q,
...
data = [q, k, v, torch.from_numpy(block_table), torch.from_numpy(context_lens), mask, kDescale, vDescale]
...
PARAM = json.dumps(
    {"headNum": 32, "qkScale": (1 / 128 ** 0.5), "kvHeadNum": 2, "maskType": 1, "compressType": 0, "quantType": 3,
     "outDataType": 27})
    ...
    def test(self):
        ...
        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM,
                                [in_tensors[0], in_tensors[1], in_tensors[2], in_tensors[3], in_tensors[4],
                                 in_tensors[5], in_tensors[6], in_tensors[7]])
