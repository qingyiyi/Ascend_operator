...
class TestPagedAttentionMLAQuant(operation_test.OperationTest):
    ...
    def group_mm_torch(self, heads, group_num, A, B, is_k):
        ...
        for i in range(group_num):
            if self.is_int8_flag:
                ...
                if is_k:
                    group_score_high = group_score_int32.to(torch.float32) * self.de_scale1_fp32[(i * group_head): (
                                                                                                                               i + 1) * group_head].reshape(
                        group_head, 1, 1).to(torch.float32)
                else:
                    group_score_high = group_score_int32.to(torch.float32) * self.de_scalev[(i * group_head): (
                                                                                                                          i + 1) * group_head].reshape(
                        group_head, 1, 1).to(torch.float32)
            else:
                group_score_high = torch.matmul(A[i * group_head: (i + 1) * group_head, :, :].to(torch.float32),
        ...
        for i in range(len(context_lens)):
            ...
            if mask_dim == 4:
                ...
                out, out_high = self.ref_masked_attention(q, keys, values, scale,
                                                          mask[i // mask_index_coff, :, :context_len], mask_data_type)
                out = out.reshape(num_heads, head_size_vo)
            else:
    ...
    def calc_data(self, num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks, k_seqlen, \
        ...
        query = torch.from_numpy(
            np.random.uniform(q_min_range, q_max_range, size=(num_tokens, num_heads, head_size_qk))).to(dtype)
        ...
        if not compressHead:
            key_cache = torch.from_numpy(np.random.uniform(kv_min_range, kv_max_range,
                                                           size=(num_blocks, block_size, kv_heads, head_size_qk))).to(
                kv_type)
    ...
    def calc_data_perform(self, num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                          k_seqlen, \
                          dtype, mask_dim=4, mask_data_type=torch.bfloat16, \
                          dynamic_batch=False, dynamic_seqlen=None, is_int8_flag=False, has_bias=False,
    ...
    def test_paged_mla_quant_online_mask0_case(self):
        ...
        for i in [1,4,16,64]:
            ...
            is_quant_flag = 1
            is_kv_combined = True
            self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                           k_seqlen, dtype, mask_dim, torch.bfloat16, is_kv_combined=is_kv_combined,
                           is_quant_flag=is_quant_flag)
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
                        "maskType": 0, "kvSeqLen": self.contex_lens.tolist(), "quantType": 3, 'outDataType': 27}

            PARAM = json.dumps(
                {"headNum": num_heads, "qkScale": float(1.0 / (head_size_qk ** 0.5)), "kvHeadNum": kv_heads,
                 "maskType": 0,
            ...
            self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.npu(), self.key_cache.npu(),
                                                                torch.from_numpy(
                                                                    self.block_tables.astype(np.int32)).npu(),
    ...
    def test_paged_mla_quant_offline_mask0_case(self):
        ...
        for i in [1,4,16,64]:
            ...
            self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                           k_seqlen, dtype, mask_dim, torch.bfloat16, is_kv_combined=is_kv_combined,
                           is_quant_flag=is_quant_flag, is_quant_offiline=is_quant_offiline)
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
                        "maskType": 0, "kvSeqLen": self.contex_lens.tolist(), "quantType": 2, 'outDataType': 27}
    ...
    def test_pa_mask_quantoffline_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
            ...
            is_quant_offiline = 1
            self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                           k_seqlen, dtype, mask_dim, torch.bfloat16, dynamic_batch=False, dynamic_seqlen=k_seqlen,
                           is_kv_combined=is_kv_combined, is_quant_flag=is_quant_flag,
                           is_quant_offiline=is_quant_offiline)
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
                        "maskType": 1, "kvSeqLen": self.contex_lens.tolist(), "quantType": 2, 'outDataType': 27}
            PARAM = json.dumps(
                {"headNum": num_heads, "qkScale": float(1.0 / (head_size_qk ** 0.5)), "kvHeadNum": kv_heads,
                 "maskType": 1,
                 "mlaVHeadSize": head_size_vo, "quantType": 2, 'outDataType': 27})
            ...
            self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.npu(), self.key_cache.npu(),
                                                                torch.from_numpy(
                                                                    self.block_tables.astype(np.int32)).npu(),
                                                                torch.from_numpy(self.contex_lens).npu(),
                                                                self.alib_mask.to(dtype).npu(),
                                                                self.de_scale1_fp32.npu(), self.de_scale2_fp32.npu(),
                                                                self.scale.npu()])

    def test_pa_mask_quant_online_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
            ...
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
                        "maskType": 1, "kvSeqLen": self.contex_lens.tolist(), "quantType": 3, 'outDataType': 27}
            PARAM = json.dumps(
                {"headNum": num_heads, "qkScale": float(1.0 / (head_size_qk ** 0.5)), "kvHeadNum": kv_heads,
                 "maskType": 1,
                 "mlaVHeadSize": head_size_vo, "quantType": 3, 'outDataType': 27})
    ...
    def test_pa_dynamic_mask_batch_quantoffline_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
            ...
            print(f'-------------batch_tatus:{batch_tatus}')
            k_seqlen = [500] * i * batch_tatus
            tor = 1.0 / (head_size_qk ** 0.5)
            mask_dim = 3
            dtype = torch.bfloat16
            is_quant_flag = 1
            ...
            is_quant_offiline = 1
            self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                           k_seqlen, dtype, mask_dim, torch.bfloat16, dynamic_batch=True, dynamic_seqlen=k_seqlen,
                           is_kv_combined=is_kv_combined, is_quant_flag=is_quant_flag,
                           is_quant_offiline=is_quant_offiline)
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
            ...
            RUN_PARAM = json.dumps({"contextLens": self.contex_lens.tolist(), "batchRunStatus": batch_tatus.tolist()})
            shape_out = ((num_tokens, num_heads, head_size_vo))
            attention_out = torch.zeros(shape_out, dtype=torch.bfloat16)
            attention_out[:] = 0.1
            self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.npu(), self.key_cache.npu(),
    ...
    def test_pa_dynamic_batch_quantoffline_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
    ...
    def test_pa_dynamic_mask_batch_quant_online_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
            ...
            self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.npu(), self.key_cache.npu(),
                                                                ...
                                                                self.de_scale1_fp32.npu(), self.de_scale2_fp32.npu()])
    #
    def test_pa_dynamic_batch_quant_online_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
    ...
    def test_paged_mla_quant_online_mask0_fp16_case(self):
        ...
        for i in [1,4,16,64]:
            ...
            is_kv_combined = True
            self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                           k_seqlen, dtype, mask_dim, torch.float16, is_kv_combined=is_kv_combined,
                           is_quant_flag=is_quant_flag)
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
                        "maskType": 0, "kvSeqLen": self.contex_lens.tolist(), "quantType": 3, 'outDataType': 27}
    ...
    def test_pa_mask_quantoffline_fp16_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
            ...
            is_quant_offiline = 1
            self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                           k_seqlen, dtype, mask_dim, torch.float16, dynamic_batch=False, dynamic_seqlen=k_seqlen,
                           is_kv_combined=is_kv_combined, is_quant_flag=is_quant_flag,
                           is_quant_offiline=is_quant_offiline)
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
    ...
    def test_pa_mask_quant_online_fp16_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
    ...
    def test_pa_dynamic_mask_batch_quantoffline_fp16_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        print('-----test_pa_quantoffline_mask_dynamic_batch_fp16_case test start-----')
        for j in [1,4,16,64]:
            num_tokens = j
            num_heads = 128
            kv_heads = 1
            ...
            dynamic_batch = True
            # batch_tatus = [1] * num_tokens
            batch_tatus = np.array([1] * num_tokens).astype(np.int32)
            ...
            is_quant_offiline = 1
            self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                           k_seqlen, dtype, mask_dim, dtype, dynamic_batch=True, dynamic_seqlen=k_seqlen,
                           is_kv_combined=is_kv_combined, is_quant_flag=is_quant_flag,
                           is_quant_offiline=is_quant_offiline)
            OP_NAME = "PagedAttentionOperation"
            OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "tor": tor, "headDimV": head_size_vo,
            ...
            # RUN_PARAM = json.dumps({"contextLens": self.contex_lens.tolist(), "batchRunStatus": batch_tatus.tolist()})
            RUN_PARAM = json.dumps({"contextLens": self.contex_lens.tolist(), "batchRunStatus": 1})
            shape_out = ((num_tokens, num_heads, head_size_vo))
            ...
            self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [self.q.npu(), self.key_cache.npu(),
                                                                ...
                                                                self.scale.npu()])
    #
    def test_pa_dynamic_batch_quantoffline_fp16_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
    ...
    def test_pa_dynamic_batch_quant_online_fp16_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
    ...
    def test_pa_dynamic_mask_batch_quant_online_fp16_case(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        for i in [1,4,16,64]:
