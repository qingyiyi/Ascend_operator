...
class TestPagedAttentionMLA(operation_test.OperationTest):
    ...
    def ref_single_query_cached_kv_attention(self,
        ...
        for i in range(len(context_lens)):
            ...
            if mask_dim == 4:
                out,out_high,sim_out = self.ref_masked_attention(q, keys, values, scale, mask[i, :, :, :context_len], mask_data_type)
                out = out.reshape(num_heads, head_size_vo)
            elif mask_dim == 3:
                out,out_high,sim_out = self.ref_masked_attention(q, keys, values, scale, mask[i // mask_index_coff, :, :context_len], mask_data_type)
                out = out.reshape(num_heads, head_size_vo)
            else:
    ...
    def test_paged_mla_combine_kv_numTokens32_numHeads32(self):
        ...
        PARAM = json.dumps({"headNum": num_heads, "qkScale":tor, "kvHeadNum":kv_heads, "maskType": 0, "cacheMode": 1})
    ...
    def test_paged_mla_combine_kv_numTokens64_numHeads32(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        num_tokens = 64
    ...
    def test_paged_mla_combine_kv_numTokens64_numHeads64(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        num_tokens = 64
    ...
    def test_paged_mla_combine_kv_numTokens64_numHeads128(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        num_tokens = 64
