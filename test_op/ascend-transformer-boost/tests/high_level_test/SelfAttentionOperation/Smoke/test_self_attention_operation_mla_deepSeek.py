...
class TestFlashAttentionPrefixEncoder(operation_test.OperationTest):
    ...
    def set_data_params(self, dynamic_batch=False, batch_state=None, window_size=0, cache_type=0,
        ...
        if num_blocks is None:
            ...
            for i in range(batch):
                ...
                block_table = [
                    random.randint(0, num_blocks - 1) if j < num_blocks_cur_seq else 0 for j in
                    range(max_num_blocks_per_seq)
                ]
    ...
    def gen_mask(self, batch, heads, data_type, mask_type, window_size, is_compress, cache_type=0):
        ...
        mask_type_dict = {
            ...
            MASK_TYPE_ALIBI_WITH_PREFIX_BATCH: ((batch, heads, q_max_seq, kv_max_seq),
                                                (lambda mask, idx, q_s, kv_s: (mask[idx, :, kv_s - q_s:kv_s, :kv_s]))),
            # 三维的alibi mask
            ...
            (heads, q_max_seq, kv_max_seq), (lambda mask, idx, q_s, kv_s: (mask[:, kv_s - q_s:kv_s, :kv_s]))),
            MASK_TYPE_NO_HEAD: (
            ...
            (1, q_max_seq, kv_max_seq), (lambda mask, idx, q_s, kv_s: (mask[:, kv_s - q_s:kv_s, :kv_s]))),
    ...
    def test_flash_attention_mla_fp16_deepseek_test1(self):
        ...
        OP_PARAM = {"type": 1, "qSeqLen": kv_seqLen, "kvSeqLen": kv_seqLen, "headDimV": embeddimV, "headSize": heads,
                    "tor": tor, "maskType": 1, "isTriuMask": 1}
        ...
        self.gen_out_tensor()
        self.mask = self.mask.view(batch, 128, 128)[0, :, :].to(data_type).npu()
        ...
        PARAM = json.dumps(
            {"headNum": heads, "calcType": 3, "maskType": 1, "kvHeadNum": kv_head, "qkScale": tor, "isTriuMask": 1})
        RUN_PARAM = json.dumps({"seqLen": kv_seqLen})

        self.execute_with_param(OP_NAME, PARAM, RUN_PARAM, [
            self.q.view(3072, 128, 192).npu(), self.k.view(3072, 128, 192).npu(), self.v.view(3072, 128, 128).npu(),
            self.mask.view(128, 128).npu(), self.layer_id.npu()])
