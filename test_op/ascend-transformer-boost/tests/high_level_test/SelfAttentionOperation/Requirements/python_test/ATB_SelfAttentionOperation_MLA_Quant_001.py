...
class TestFlashAttentionQuant(operation_test.OperationTest):
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_online(self):
        ...
        for i in random.sample(range(1, 65), 3):
            ...
            OP_PARAM = {"type": 2010, "qSeqLen": kv_seqLen, "kvSeqLen": kv_seqLen, "headDimV": embeddimV,
            ...
            self.set_data_params(dynamic_batch=dynamic_batch,
                                 ...
                                 op_type=OP_PARAM["type"], mask_type=MASK_TYPE_NO_HEAD, no_cache=True, fav3=fav3,
                                 tor=tor)
            self.gen_out_tensor(online)
            PARAM = json.dumps(
                {"headNum": heads, "qkScale": float(1.0 / math.sqrt(1.0 * self.embeddim)), "kvHeadNum": kv_head, \
                 "calcType": 3, "maskType": 1, "mlaVHeadSize": embeddimV, "kernelType": 0, "quantType": 3,
                 "outDataType": 27})
