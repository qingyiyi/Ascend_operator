...
from PagedAttentionOperation.Requirements.python_test.precision_calcu import *
...
class TestFlashAttention():
    ...
    def test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_offline(self):
        ...
        self.set_data_params(dynamic_batch = dynamic_batch, 
                             ...
                             op_type = 10, mask_type = MASK_TYPE_NO_HEAD, no_cache = True, fav3 = fav3, is_mask = True, tor = tor)

        self.gen_out_tensor()
        attention_out = np.zeros_like(self.q.to(torch.float16))
        if not self.fav3:
            ret = [self.q, self.k, self.v, self.mask.to(data_type), self.kv_seqLen, torch.tensor(self.golden_out, dtype=torch.bfloat16)]
            return ret
        else:
            print
            ret = [self.q_int8, self.k_int8, self.v_int8, self.mask.to(data_type), self.kv_seqLen, torch.tensor(self.q_scale, dtype=torch.float), 
                            torch.tensor(self.q_offset, dtype=torch.int32), torch.tensor(self.v_scale, dtype=torch.float), 
                            torch.tensor(self.v_offset, dtype=torch.int32), torch.tensor(self.offline_scale, dtype=torch.float), torch.tensor(self.golden_out, dtype=torch.bfloat16)]
            return ret
...
data = data_generator.test_flash_attention_case_fa_encoder_nocache_bf16_fav3_int8_offline()
param_seqlen = data[4]
...
PARAM = json.dumps({"headNum": 2, "qkScale": tor, "kvHeadNum": 2,
                    "calcType": 3, "maskType": 1, "isTriuMask": 0, "kernelType": 0, "quantType": 2, "outDataType": 27})
RUN_PARAM = json.dumps({"seqLen": param_seqlen, "quantType": 2})

class TestFlashAttentionEncoderOperationQKVQuantFp16Offline(operation_test.OperationTest):
    def golden_calc(self, input_tensors):
        return [in_tensors[10]]

    def golden_compare(self, out_tensor, golden_out_tensor):
        result_single = data_generator.compare_output_data(out_tensor.cpu(), golden_out_tensor.cpu(), [0.001, 0.001, 0.005, 0.005])
