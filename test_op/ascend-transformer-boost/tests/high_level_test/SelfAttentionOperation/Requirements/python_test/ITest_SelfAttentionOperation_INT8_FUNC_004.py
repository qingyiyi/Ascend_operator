...
class TestFlashAttention():
    ...
    def test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_offline(self):
        ...
        if not self.fav3:
            ret = [self.q, self.k, self.v, self.mask.to(data_type), self.kv_seqLen, self.golden_out]
            return ret
        else:
            ret = [self.q_int8, self.k_int8, self.v_int8, self.mask.to(data_type), self.kv_seqLen, torch.tensor(self.q_scale, dtype=torch.float), 
                            ...
                            torch.tensor(self.v_offset, dtype=torch.int32), torch.tensor(self.offline_scale, dtype=torch.float), self.golden_out]
            return ret


data_generator = TestFlashAttention()

data = data_generator.test_flash_attention_case_fa_encoder_nocache_fp16_fav3_int8_offline()
param_seqlen = data[4]
...
PARAM = json.dumps({"headNum": 2, "qkScale": tor, "kvHeadNum": 2,
                    "calcType": 3, "maskType": 1, "isTriuMask": 0, "kernelType": 0, "quantType": 2, "outDataType": 1})
