...
class TestPagedAttentionMLA(operation_test.OperationTest):
    ...
    def test_paged_mla_combine_kv_blcok_size_256_fp16(self):
        ...
        kv_heads = 1
        # 模型shape
        for num_tokens in [32, 64]:
            for num_heads in [16,32]:
                block_size = 256
                head_size_qk = 576
                head_size_vo = 512
                num_blocks = 64
                k_seqlen = [512] * num_tokens
                k_seqlen = 512
                # for k_seqlen in [ 256, 384, 512, 800, 1024]:
                # for k_seqlen in [512]:
    ...
    def test_paged_mla_combine_kv_blcok_size_256_bf16(self):
        if not operation_test.get_soc_version() == 'Ascend910B':
            print("this testcase only supports Ascend910B")
            return
        num_tokens = 1
        ...
        for num_tokens in [32, 64]:
            for num_heads in [16, 32]:
                ...
                self.calc_data(num_tokens, num_heads, kv_heads, head_size_qk, head_size_vo, block_size, num_blocks,
                               k_seqlen, dtype, mask_dim, torch.float16,
                               is_kv_combined=is_kv_combined)

                OP_NAME = "PagedAttentionOperation"
                OP_PARAM = {"type": 2005, "kvHead": kv_heads, "headSize": num_heads, "headDimV": head_size_vo,
                            "tor": tor, "maskType": 0, "kvSeqLen": self.contex_lens.tolist()}
