...
Status MatchAtTensorInt(float atol, float rtol, at::Tensor &out, at::Tensor &gt)
{
...
static Status ReduceMinGolden(float atol, float rtol, const Mki::Test::GoldenContext &context)
{
    ...
    for (int64_t i = param.axis.size()-1; i >=0; i--) {
        ...
        output_tensor = std::get<0>(torch::min(output_tensor, axis, false));
    }

    return MatchAtTensorInt(atol, rtol, atOutTensor, output_tensor);
}

TEST(TestOpReduceMin, TestMinInt32Case0)
{
    CHECK_DEVICE_VERSION_NOT_ASCEND910A();
    CHECK_DEVICE_VERSION_NOT_ASCEND310B();
    Mki::Test::MkiOpTest opTest;
    opTest.Golden(std::bind(&ReduceMinGolden, ATOL, RTOL, std::placeholders::_1));
    opTest.IntRand(0, 100);
    OpParam::Reduce opParam;
    opParam.reduceType = OpParam::Reduce::REDUCE_MIN;
    opParam.axis = {1};
...
TEST(TestOpReduceMin, TestMinInt32Case1)
{
    CHECK_DEVICE_VERSION_NOT_ASCEND910A();
    CHECK_DEVICE_VERSION_NOT_ASCEND310B();
    Mki::Test::MkiOpTest opTest;
...
TEST(TestOpReduceMin, TestMinInt32Case2) // axis输入错误，导致inferShape失败
{
    CHECK_DEVICE_VERSION_NOT_ASCEND910A();
    CHECK_DEVICE_VERSION_NOT_ASCEND310B();
    Mki::Test::MkiOpTest opTest;
...
TEST(TestOpReduceMin, TestMinInt32Case3)
{
    CHECK_DEVICE_VERSION_NOT_ASCEND910A();
    CHECK_DEVICE_VERSION_NOT_ASCEND310B();
    Mki::Test::MkiOpTest opTest;
