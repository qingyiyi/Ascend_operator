...
#include "gmm_deq_swiglu_quant_gmm_deq_tiling_data.h"
#include "gmm_deq_swiglu_quant_gmm_deq_tiling.h"

namespace AtbOps {
Status GmmDeqSwigluQuantGmmDeqTiling(const LaunchParam &launchParam, KernelInfo &kernelInfo)
{
    ...
    const size_t groupListIndex = 4;

    const size_t mDimInX1 = 0;
    const size_t kDimInX1 = 1;
    const size_t nDimInScale1 = 1;
    int64_t groupCount = launchParam.GetInTensor(groupListIndex).desc.dims.at(0);
    ...
    auto tilingDataPointer = reinterpret_cast<GmmDeqSwigluQuantGmmDeqTilingData *>(kernelInfo.GetTilingHostAddr());
    MKI_CHECK(tilingDataPointer, "tilingData should not be empty", return Status::FailStatus(ERROR_INVALID_VALUE));
    tilingDataPointer->groupCount = static_cast<uint32_t>(groupCount);
