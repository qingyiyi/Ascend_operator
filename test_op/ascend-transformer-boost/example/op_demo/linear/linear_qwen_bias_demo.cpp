...
const uint32_t X_DIM_0 = 1024;
const uint32_t X_DIM_1 = 5120;
const uint32_t WEIGHT_DIM_0 = 5120;
const uint32_t WEIGHT_DIM_1 = 896;
const uint32_t BIAS_DIM_0 = 896;
...
{
    // 创建shape为[1024, 5120]的输入x tensor
    atb::Tensor xFloat;
    ...
    // 创建shape为[5120, 896]的输入weight tensor
    atb::Tensor weightFloat;
    CHECK_STATUS(CreateTensorFromVector(contextPtr, stream, std::vector<float>{1, 2, 3, 4, 5, 6}, ACL_BF16,
                                        aclFormat::ACL_FORMAT_FRACTAL_NZ, {WEIGHT_DIM_0, WEIGHT_DIM_1}, weightFloat));
    // 创建shape为[896]的输入bias tensor
    atb::Tensor biasFloat;
    CHECK_STATUS(CreateTensorFromVector(contextPtr, stream, std::vector<float>(BIAS_DIM_0, 1.0), ACL_BF16,
