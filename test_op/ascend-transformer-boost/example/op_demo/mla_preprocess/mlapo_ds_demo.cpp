                             ...
                             atb::SVector<atb::Tensor> *inTensors)
{
    ...
    atb::Tensor wuk;
    CHECK_STATUS(CreateTensorFromVector(contextPtr, stream, std::vector<float>(headNum * 128 * 512), dtype,
                                        aclFormat::ACL_FORMAT_ND, {headNum, 128, 512}, wuk));
    // 创建shape为[blockNum, block_size, 1, 512]的输入kvCache tensor
    atb::Tensor kvCache;
    CHECK_STATUS(CreateTensorFromVector(contextPtr, stream, std::vector<float>(blockNum * 512 * blockSize * 1), dtype,
                                        aclFormat::ACL_FORMAT_ND, {blockNum, blockSize, 1, 512}, kvCache));
    ...
    CHECK_STATUS(CreateTensorFromVector(contextPtr, stream, std::vector<float>(blockNum * blockSize * 64, 0), dtype,
                                        aclFormat::ACL_FORMAT_ND, {blockNum, blockSize, 1, 64}, kvCacheRope));
